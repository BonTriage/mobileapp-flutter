...
class CalendarUtil {
  ...
  List<Widget> drawMonthCalendar(
    ...
    var dateUtility = CalendarUtil();
    var daysInMonth = dateUtility.daysInMonth(mm, yy);
    var weekDay = _firstDateOfMonth.weekday != 7?_firstDateOfMonth.weekday:0;
    ...
    for (int n = 0,i = 0; n < 37 && i<daysInMonth ; n++) {
        if(n<weekDay || n > daysInMonth + weekDay){
          ...
          if (currentWeekConsData[i] && (n + 1) % 7 != 0 && n<daysInMonth) {
            var j = i + 1;
            if (j < daysInMonth && currentWeekConsData[i] == currentWeekConsData[j]) {
              monthData.add(
                  ConsecutiveSelectedDateWidget(
                      _firstDateOfMonth.day.toString()));
            } else {
              monthData
                  .add(DateWidget(_firstDateOfMonth.day.toString()));
            }
            i++;
          } else {
            monthData.add(DateWidget(_firstDateOfMonth.day.toString()));
            i++;
          }
